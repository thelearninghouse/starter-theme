    ...
    class PMXE_Updater
    {
        ...
        public function show_update_notification($file, $plugin)
        {
            ...
            if (!empty($update_cache->response[$this->name]) && version_compare($this->version, $version_info->new_version, '<')) {
                ...
                $new_version_number = $version_info->new_version;

                $isBeta = false;

                if(strpos($version_info->new_version, "-beta-") !== false) {
                    $isBeta = true;
                }

                if(version_compare($new_version_number, '1.5.5') >= 0 && !$isBeta) {
                    $notifications = new \Wpae\App\Migration\Notifications();
                    $new_version .= $notifications->getNotification(\Wpae\App\Migration\Notifications::NOTIFICATION_ON_PLUGINS_PAGE);
                }
            }
